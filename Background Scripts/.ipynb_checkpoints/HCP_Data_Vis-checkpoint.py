...
def Plot_Brain_Net(Network_property, path_pos=path_pos, scale=1,*args):
    """ Brain 3D plot with network property-dependent node size and color
    
    Parameters
    ----------
    Network_property: list
        A list of values with node property
    ...
    sizec = 2 * scale * np.array(Network_property)
    # restriction in the size
    if max(sizec)>14:
        sizec=14/max(sizec)*sizec
    if args:
        colorV=args[0]
    else:
        colorV = np.array(Network_property)
...
def plot3d(matrix, e, scale=1, verbose=False,*args):
    """This function plots one brain network in the example, with a given treshold e
    ...
    sizec = 10 * scale * [i[1] for i in G.degree()]
