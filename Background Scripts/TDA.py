...
def max_cliques(N, k):
    ...
    OBS:
    ---
    The clique algorithm is time consuming (NP) large and dense matrixes, this function is an attempt to deal with it
...
def Eulerange_thr(i, maxvalue):
    ...
    Notice that we sliced the network in 1/100 steps. We can always do higher or lower steps if required
...
def Curv_thr(e, i, verbose=False):
    ...
    nor = sum(total) #### NOTICE THAT nor AND nor2 IS NOT BEING USED - HOWEVER, IF ONES DECIDES FOR NORMALIZATION- SHOULD TAKE THIS INTO ACCOUNT
    nor2 = max(total) 
    # nt is normalized by the sum
    #nt2 is normalized by the max"
    nt = []
...
def Participation_in_cliques(d, i, cl, verbose=False):
    ...
    maxk = max(klist)
    totk = sum(klist)
    # We can do several choices on the particiaption rank: here we choose the percentage of all k-cliques
